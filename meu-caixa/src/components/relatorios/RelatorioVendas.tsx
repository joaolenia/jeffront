...
export function RelatorioVendas() {
  ...
  // Estado de Impressão
  const [saleToPrint, setSaleToPrint] = useState<Venda | null>(null);
  ...
  const handlePrintSale = (venda: Venda) => {
    // Define a venda que será impressa e aguarda o React renderizar o CupomFiscal
  ...
  return (
    <div className="relatorio-vendas">
      ...
      {saleToPrint && (
        <CupomFiscal 
          itens={saleToPrint.itens || []} 
          subtotal={Number(saleToPrint.total)} // Recibos antigos não terão o detalhamento do desconto
          desconto={0} 
          total={Number(saleToPrint.total)} 
          formaPagamento={saleToPrint.formaPagamento}
          valorRecebido={Number(saleToPrint.valorRecebido)}
          troco={Number(saleToPrint.troco)}
          dataVenda={saleToPrint.dataHora || (saleToPrint as any).data_hora} // <- PASSANDO A DATA
        />
      )}

      {/* TODO O CONTEÚDO OCULTO NA IMPRESSÃO */}
      <div className="rv-content hide-on-print">
        {/* FILTROS */}
                ...
                <div key={venda.id} className="sale-card">
                  <div className="sale-card-header">
                    ...
                    <div className="sale-header-actions">
                      ...
                      {/* Botão de Imprimir */}
                      <button className="btn-print-sale" onClick={() => handlePrintSale(venda)} title="Imprimir Recibo">
                        <Printer size={16} />
                      </button>
      ...
      {saleToDelete !== null && (
        <div className="rv-modal-overlay" onMouseDown={(e) => {
          ...
          <div className="rv-alert-box hide-on-print">
