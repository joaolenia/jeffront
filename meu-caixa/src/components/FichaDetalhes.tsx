...
import {
  ...
  ShoppingBag, Smartphone, Trash2, TrendingUp, User, Wallet
} from 'lucide-react';
import api from '../api';
              ...
              <div className="fd-table-responsive"><table className="fd-table">
                ...
                <tbody>{ficha.compras.map((compra, i) => (
                  <tr key={i}>
                    ...
                    <td><div className="fd-item-cell"><div className="fd-item-icon"><ShoppingBag size={15} /></div>{compra.resumoItens || 'Itens diversos'}</div></td>
                    <td className="text-right"><strong className="fd-table-value">{formatCurrency(compra.valor)}</strong></td>
                  </tr>
                ))}</tbody>
              </table></div>
